%% This program finds population on dynamic retention values
    clear all %vars -except EPS
tic

EPSval=1;
for EPS=0.005:0.005:0.025
...
du=10;
if u>100*du
x=sum(abs(vec_Altrusm(v,u-du:u)-vec_Altrusm(v,u-2*du:u-du))/du);
...
for V = 1:v
    plot(0:uval(V)-1,vec_Altrusm(V,1:uval(V)),'b-');
    AltEnd_vec(EPSval,V)=vec_Altrusm(V,uval(V));
end
EPSval=EPSval+1;
    clearvars -except EPS AltEnd_vec EPSval
end
toc
